...
try {
  execSync(
    `git clone --depth 1 --branch ${args.ref} ${args.repo} ${repoDir}`,
    { stdio: 'inherit' },
  );
  ...
  for (const name of mdFiles) {
    ...
    if (fm['layout']?.toLowerCase() !== 'bidder')      { skipped++; continue; }
  ...
  console.log(`wrote ${codesWithSchema.size - 1} pbjs schemas under ${pbjsDir} (${noSchema} skipped – no params; ${skipped} non-bidder files ignored)`);
...
function mapPrimitive(t) {
  switch (t) {
    ...
    case 'float': case 'number': case 'double': return 'number';
...
function writeManifest(path, docsCommit, codes, codesWithSchema) {
  ...
  /** @type {Record<string,unknown>} */
  const existingBidders = (existing.bidders && typeof existing.bidders === 'object')
